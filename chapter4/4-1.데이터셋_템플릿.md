...
> - 영상 데이터와 라벨 데이터의 파일명은 동일해야 합니다.
> - Segmentation 데이터셋과 구분하기 위해 Transformation.xlsx 파일이 반드시 존재해야 합니다.
> - Transformation.xlsx 파일안에는 아무것도 입력되지 않아도 됩니다.
> - 3D형 데이터 포맷인 .nii 혹은 .nii.gz 형태의 NIFTI 파일만 지원합니다.
...
> - 영상 데이터와 라벨 데이터가 각각 Dataset 폴더와 Label 폴더에 존재하며, 각 환자의 Patient ID 별로 폴더가 구분된 경우입니다.  
> - 영상 데이터와 라벨 데이터의 폴더명은 동일해야 합니다.
> - Segmentation 데이터셋과 구분하기 위해 Transformation.xlsx 파일이 반드시 존재해야 합니다.
> - Transformation.xlsx 파일안에는 아무것도 입력되지 않아도 됩니다.
> - 3D 데이터의 경우, 하나의 Patient 폴더안에 여러 파일들(ex. jpg, dcm)을 합쳐 하나의 3D 데이터로 구성할 수도 있고, 하나의 파일(ex. NIFTI 파일)로 3D 데이터를 구성할 수도 있습니다.
...
#### 구조3. StudyUID > SeriesUID 구조로, 모든 Series를 하나의 data로 처리하는 경우

![](img/4-1/manual_4-1_4_3.jpg)

> - 영상 데이터와 라벨 데이터가 각각 Dataset 폴더와 Label 폴더에 존재하며, 각 환자의 Study UID 별로 폴더가 구분되어 있고 Study UID 아래에 다양한 Series 별로 구분이 되어있는 경우입니다.  
> - Series 파일들이 channel 축으로 쌓여 데이터를 구성합니다. 
>- 영상 데이터와 라벨 데이터의 Study UID명은 동일해야 합니다.
> - Segmentation 데이터셋과 구분하기 위해 Transformation.xlsx 파일이 반드시 존재해야 합니다.
> - Transformation.xlsx 파일안에는 아무것도 입력되지 않아도 됩니다.
> - 3D 데이터의 경우, 하나의 Patient 폴더안에 여러 파일들(ex. jpg, dcm)을 합쳐 하나의 3D 데이터로 구성할 수도 있고, 하나의 파일(ex. NIFTI 파일)로 3D 데이터를 구성할 수도 있습니다.
><br/>