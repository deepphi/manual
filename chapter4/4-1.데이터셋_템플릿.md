...
각 클래스를 컬럼으로 나열하고 해당되는 경우 1, 아닌 경우를 0으로 **한 개의 데이터가 여러 클래스에 속하는 경우 사용하는 구조**입니다.
- 3D 데이터의 경우 filename 대신 patient id가 사용됩니다.

### (2) Segmentation 데이터셋 구조

#### 1) Dataset/Label 폴더에 영상이 있는 경우
...
#### 3) StudyUID > SeriesUID 구조로, 각 Series를 하나의 data로 처리하는 경우
...
#### 4)  StudyUID > SeriesUID 구조로, 모든 Series를 하나의 data로 처리하는 경우
...
#### 5) 위 경우 중 Label 파일이 다수이며 개수가 다른 경우
...
#### 3) StudyUID > SeriesUID 구조로, 모든 Series를 하나의 data로 처리하는 경우
